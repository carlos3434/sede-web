...
export default {
  props: {
    title: {
      type: String,
      default: 'Adjuntar Anexo Nº 9'
    }
  },
  data() {
    return {
      modalId: 'modal-anexo9',
      openModalEvent: 'open-anexo9-modal',
      refreshTableEvent: 'refresh-table',
      apiUrlPartial: '/diligenciaverificador/',
      msgSuccess: 'Anexo Nº 9 subido exitosamente!!!',
      item: {
        diligencia_id: null,
        anexo_file: null
      }
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs.form.validate().then((success) => {
        ...
        let apiUrl = this.apiUrlPartial + this.item.diligencia_id + '/updateAnexo9'

        let data = new FormData()
        data.append('anexo9', this.item.anexo_file)
          ...
          <div class="row">
            <div class="col-12">
              <ValidationProvider rules="required|ext:pdf|size:3072" name="Anexo" v-slot="{ validate, errors }">
                <b-form-group label="Anexo Nº 9.PDF:" class="mb-3">
