...
export default {
  props: {
    title: {
      type: String,
      default: 'Eliminar Experiencia Como Inspector'
    }
  },
  data() {
    return {
      ...
      msgSuccess: 'Experiencia Como Inspector eliminada exitosamente!!!',
      isProcessing: false,
      item: {
        id: null,
        text: ''
  ...
  methods: {
    ...
    setItemValues(item) {
      ...
      this.item.text = item.institucion
    },
    resetFormValues() {
      // Resetting Values
      this.item.id = null
...
<template>
  <div>
    <b-modal :id="modalId" :title="title" @ok="handleOk" @hidden="hideModal">
      <p>¿Está seguro que desea eliminar esta experiencia?</p>
      <p>
        <b>{{ item.text }}</b>
      </p>
