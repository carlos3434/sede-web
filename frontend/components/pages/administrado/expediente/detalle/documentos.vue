<script>
export default {
  props: {
    id_expediente: {
      type: String,
      default: null
    },
    estado_expediente: {
      type: String,
      default: null
    },
    categoria: {
      type: Object,
      default: {}
    },
    visible: {
      type: Boolean,
      default: false
    },
    numero: {
      type: String,
      default: '00'
    }
  },
  data: () => ({
    fields: [
      {
        label: 'Documento',
        key: 'nombre_archivo',
        sortable: false,
        visible: true
      },
      {
        label: 'Estado',
        key: 'estado_revision_nombre',
        sortable: false,
        visible: true,
        class: 'bs-option-col',
        thClass: 'bs-option-col-header'
      },
      {
        label: 'Archivo',
        key: 'valor_archivo',
        sortable: false,
        visible: true,
        class: 'bs-option-col',
        thClass: 'bs-option-col-header'
      },
      {
        label: 'Opciones',
        key: 'opciones',
        sortable: false,
        visible: false,
        class: 'bs-option-col',
        thClass: 'bs-option-col-header'
      }
    ]
  }),
  computed: {
    visibleFields() {
      this.fields.filter((f) => {
        if (f.key === 'opciones') {
          switch (this.estado_expediente) {
            case 'CREADO':
              f.visible = true
              break
            case 'OBSERVADO':
              f.visible = true
              break
            default:
              f.visible = false
              break
          }
        }
      })
      return this.fields.filter((f) => f.visible)
    }
  },
  methods: {
    refreshTable() {
      this.getExpedienteData()
    },
    requestEditItem(item) {
      item.id_expediente = this.id_expediente
      this.$nuxt.$emit('open-edit-modal', item)
    },
    requestDeleteItem(item) {
      item.id_expediente = this.id_expediente
      this.$nuxt.$emit('open-delete-modal', item)
    },
    colorEstadoDocumento(itemStadus) {
      let color = 'info'

      switch (itemStadus) {
        case 'OBSERVADO':
          color = 'danger'
          break
      }

      return color
    },
    checkStatusToEdit(itemStadus) {
      if (this.estado_expediente == 'CREADO') {
        return true
      }
      return itemStadus == 'OBSERVADO'
    }
  }
}
</script>

<template>
  <div class="card" style="border-top: 2px solid #50a5f1">
    <a href="javascript: void(0);" class="text-dark" data-toggle="collapse" v-b-toggle="`documentos-` + numero">
      <div class="p-2">
        <div class="media align-items-center">
          <div class="me-3">
            <div class="avatar-xs">
              <div class="avatar-title rounded-circle bg-soft-primary text-primary">{{ numero }}</div>
            </div>
          </div>
          <div class="media-body overflow-hidden">
            <h5 class="font-size-16 mb-1">
              {{ categoria.nombre }}
            </h5>
            <p class="text-info text-truncate mb-0">
              <b>{{ categoria.estadisticas.completados }}</b>
              de
              <b>{{ categoria.estadisticas.total }}</b>
              documentos cargados
            </p>
          </div>
          <i class="mdi mdi-chevron-up accor-down-icon font-size-24"></i>
        </div>
      </div>
    </a>

    <b-collapse :id="`documentos-` + numero" :visible="visible">
      <div class="p-4 border-top">
        <b-table hover :items="categoria.archivos" :fields="visibleFields">
          <template #cell(valor_archivo)="data">
            <template v-if="data.value">
              <a href="javascript:void(0);" class="text-decoration" @click.stop="$downloadFile(data.value)">
                <i class="uil uil-file-info-alt font-size-18"></i> Ver Documento
              </a>
            </template>
            <template v-else>
              <span class="text-danger"> <i class="uil uil-multiply font-size-18"></i> Sin documento </span>
            </template>
          </template>
          <template v-slot:cell(estado_revision_nombre)="data">
            <template v-if="data.item.valor_archivo && data.value">
              <span class="badge font-size-12" :class="`bg-soft-` + colorEstadoDocumento(data.value)">
                {{ data.value }}
              </span>
            </template>
          </template>
          <template v-slot:cell(opciones)="data">
            <ul class="list-inline two-options mb-0" v-if="checkStatusToEdit(data.item.estado_revision_nombre)">
              <li class="list-inline-item">
                <a
                  href="javascript:void(0);"
                  class="px-1 text-primary"
                  v-b-tooltip.hover
                  title="Editar"
                  @click="requestEditItem(data.item)"
                >
                  <i class="uil uil-pen font-size-18"></i>
                </a>
              </li>
              <li class="list-inline-item" v-if="data.item.valor_archivo">
                <a
                  href="javascript:void(0);"
                  class="px-1 text-danger"
                  v-b-tooltip.hover
                  title="Eliminar"
                  @click="requestDeleteItem(data.item)"
                >
                  <i class="uil uil-trash-alt font-size-18"></i>
                </a>
              </li>
            </ul>
          </template>
        </b-table>
      </div>
    </b-collapse>
  </div>
</template>
