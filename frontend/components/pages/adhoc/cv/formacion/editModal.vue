...
export default {
  ...
  props: {
    title: {
      type: String,
      default: 'Editar Formación Académica'
    }
  },
  data() {
    return {
      ...
      msgSuccess: 'Formación Académica actualizada exitosamente!!!',
      isProcessing: false,
      item: {
        id: null,
        especialidad: '',
    ...
    <b-modal :id="modalId" :title="title" @ok="handleOk" size="lg" @hidden="resetModal">
      <ValidationObserver ref="form" v-slot="{ handleSubmit }" @submit.prevent>
        <b-form @submit.prevent="handleSubmit(onSubmit)">
          <ValidationProvider
            mode="eager"
            rules="required|alpha_num_spaces|max:120"
            name="Especialidad"
          ...
          <div class="row">
            <div class="col-6">
              <ValidationProvider
                mode="eager"
                rules="required|alpha_num_spaces|max:80"
                name="Ciudad"
                v-slot="{ validated, valid, errors }"
              >
