...
export default {
  ...
  data() {
    return {
      ...
      msgSuccess: 'Documento eliminado exitosamente!!!',
      isProcessing: false,
      item: {
        id: null,
        text: ''
  ...
  computed: {
    apiUrl: function () {
      return '/expedienteadhoc/' + this.item.id_expediente + '/archivo/' + this.item.id
    }
  },
  methods: {
    refreshTable() {
    ...
    setItemValues(item) {
      ...
      this.item.text = item.nombre_archivo
    },
    resetFormValues() {
      // Resetting Values
      this.item.id = null
